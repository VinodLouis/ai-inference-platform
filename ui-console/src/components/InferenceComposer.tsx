...
import type { InferenceComposerFormValues, InferenceJob, ModelInfo } from '../types'
...
export default function InferenceComposer ({
  ...
  async function submitInference (values: InferenceComposerFormValues): Promise<void> {
    try {
      ...
      onJobCreated(buildOptimisticJob({
        jobId: result.jobId,
      ...
      message.error(apiError.response?.data?.error || 'Inference submission failed')
    }
  }
  ...
  return (
    ...
    >
      <Form.Item label='Available Models' name='modelId' rules={[{ required: true }]}>
        <Select
          ...
          dropdownRender={(menu) => (
            <>
              ...
              <Space style={{ padding: '0 8px 8px' }}>
                <Button size='small' onClick={onRefreshModels} loading={modelsLoading}>
                  Refresh Models
                </Button>
              </Space>
            </>
