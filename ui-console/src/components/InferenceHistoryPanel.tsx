...
import { formatMultilineOutput, getStatusTagColor, shortJobId, updateJobFromStatusResponse } from '../utils/inference'
...
export default function InferenceHistoryPanel ({
  ...
  const checkJobStatus = useCallback(async (jobId: string) => {
    ...
    try {
      const response: InferenceStatusResponse = await fetchInferenceStatus(jobId)
      const now = Date.now()

      onHistoryChange((prev) => prev.map((item) => {
        ...
        columns={[
          {
            ...
            width: 120,
            render: (value: string) => (
              <Text code title={value}>{shortJobId(value)}</Text>
            )
          },
          {
            title: 'Model',
            ...
            width: 220,
            render: (value: string) => (
              <Text style={{ display: 'inline-block', maxWidth: 200 }} ellipsis={{ tooltip: value }}>
                {value || '-'}
              </Text>
            )
          },
          ...
          },
          {
            ...
            dataIndex: 'updatedAt',
            width: 220,
            render: (value: string | number) => value ? new Date(value).toLocaleString() : '-'
          },
          {
            title: 'Action',
            key: 'action',
            ...
            render: (_: unknown, record: InferenceJob) => (
              <Button size='small' loading={checkLoading} onClick={() => checkJobStatus(record.jobId)}>
                Check
              </Button>
            )
          }
      ...
      >
        {detailJob && (
          <Descriptions size='small' bordered column={1}>
            <Descriptions.Item label='Job ID'>{detailJob.jobId}</Descriptions.Item>
            <Descriptions.Item label='Model'>{detailJob.modelId}</Descriptions.Item>
            <Descriptions.Item label='Rack'>{detailJob.rackId}</Descriptions.Item>
